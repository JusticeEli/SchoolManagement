...
*/
    ...
    val PICK_USER_PROFILE_IMAGE = 1000
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode === RESULT_OK) {
            if (requestCode === PICK_USER_PROFILE_IMAGE) {
                if (resultCode === RESULT_OK) {
                   // val bmp = ImagePicker.getImageFromResult(requireContext(), resultCode, data)
                    //your compressed rotated bitmap here
                    val bmp = data!!.extras!!.get("data") as Bitmap //this is your bitmap image and now you can do whatever you want with this
    ...
    private fun startProcess(bitmap: Bitmap) {
        Log.d(TAG, "startProcess: ")
        CoroutineScope(IO).launch {
            ImageTextReader.readTextFromImage2(bitmap) {
                Log.d(TAG, "startProcess: it:$it")
            }
        }
